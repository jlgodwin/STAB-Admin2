...
nat_1year <- function(svy_data,year,
  ...
  age_data<-data.frame(svy_data$b3,svy_data$b7,svy_data$v008,svy_data$v025,
                       svy_data$v005,svy_data$v021,svy_data$v022,svy_data$v024)
  
  colnames(age_data)<-c("b3","b7","v008","v025","v005",'v021','v022',"v024")
  age_data$v022 <- as.character(interaction(age_data$v024, age_data$v025))
  # age_data$v024 <- NULL
...
pre_prop<-function(svy_data_list,loop_up,svy_year_vec){
  ...
  for (i in 1:length(svy_data_list)){
    ...
    br_temp<-data.frame(temp_fl$b3,temp_fl$b7,temp_fl$v008,
                        ...
                        temp_fl$v022,temp_fl$v001,temp_fl$v024)
    colnames(br_temp)<-c("b3","b7","v008","v025","v005",'v021','v022',
                         "v001","v024")
    br_temp$v022 <- as.character(interaction(br_temp$v024,br_temp$v025))
    # br_temp$v024 <- NULL
