#### Admin0 ####
adm0.compare <- data.frame(GADM = "Eswatini",
                           IHME = "Swaziland")
...
adm2.compare.new <- adm2.compare

#taylor edited


for (i in 1:nrow(adm2.compare)) {
  temp <- which(adm2.compare$GADM[i] %>% as.character == adm2.compare$IHME)
  if (length(temp) == 0) {
    adm2.compare.new$IHME[i]  <- NA
  } else {
    adm2.compare.new$IHME[i] <- adm2.compare$IHME[temp] %>% as.character()
  }
  
}
adm2.compare <- adm2.compare.new
