...
#Neighbors defined by distance rather than adjacency (no neighbors otherwise)
as.matrix(gDistance(poly.adm1, byid=TRUE))

#if(!file.exists(paste0(poly.path,'/', country, '_Amat.rda'))){
adm1.ind <- exists("poly.adm1")
adm2.ind <- exists("poly.adm2")
if(adm1.ind){
  admin1.mat <- poly2nb(SpatialPolygons(poly.adm1@polygons), snap=0.4)
