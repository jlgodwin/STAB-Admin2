...
if(usingGoogleSheets){
  ...
  CountryList <- read.csv(paste0(data.dir,
                                 'CountryList.csv'))
  SurveyInfo <- read.csv(paste0(data.dir,
                                'SurveyInfo.csv'))
}
...
message(cat(country, " has ",
            length(surveys),
            "surveys in years ",
            surveys,".\n"))

births.files <- SvyIds.idx <-
  rep(NA, n.survey)
...
for(i in 1:n.survey){
  ...
  births.files[i] <- unique(paste0(br.name.root, "DT/", br.name.root, "FL.dta"))
...
table(poly.adm1$TYPE_1)
table(poly.adm2$TYPE_2)

message("This might be different for everyone! \n 
        If any of the TYPE_1 or TYPE_2 variables in 
        poly.adm1 or poly.adm2, respectively, 
        contain water bodies, remove them. \n")
...
if(adm2.ind){
  ...
  colnames(admin2.mat) <- 
    rownames(admin2.mat) <- paste0("admin2_",
                                   1:dim(admin2.mat)[1])
...
for(survey in surveys){
  svy.idx <- svy.idx + 1
  dat.tmp <- getBirths(filepath = paste0(folder.name,
                                         '/dhsStata/',
                                         filenames[svy.idx]),
                       surveyyear = survey,
  ...
  cat("\n Made cluster x age x time frame for survey", 
      svy.idx,"\n")
  
  ## Assign Lat & Long ####
  
  points.path <- paste0(folder.name,
                        "/dhsFlat/",
                        points.files[[svy.idx]])
  ...
  if(adm2.ind){
    ...
    if(dim(miss.frame.adm2)[1] != 0){
      ...
      if(sum(miss.poly.adm2[1] > 100) != 0){
        warning('Check on weird long lats for clusters manually!!')
      }
  ...
  cat("\n Made Admin1 and Admin2 assignments
      for survey ", svy.idx, ".")
  png(paste0(folder.name,
             '/Plots/ShapeCheck/Points_',
             survey, '_BR.png'))
...
message('If there are no deaths or  
        an extremely small amount of person months,
        remove the most recent year.\n')
# mod.dat <- mod.dat[mod.dat$years !=
#                      max(as.numeric(as.character(mod.dat$years))),]

## Save frame ####


## Remove cluster from 2018 DHS that is outside country boundary.
points2018 <- readOGR(dsn = paste0(folder.name, 
                                   '/dhsFlat/',
                                   gps.files[match(2018,
                                                   surveys)]),
                      layer = gps.files[match(2018,
                                              surveys)])

hist(points2018@coords[,1],
     xlab = 'Longitude',
     main = '')
hist(points2018@coords[,2],
     xlab = 'Latitude',
     main = '')

which(points2018@coords[,2] > -5)
which(points2018@coords[,1] < 10)
weird.pts <- which(points2018@coords[,1] < 10)

plot(points2018, pch = 16,
     col = alpha('grey80', 0.5))
points(points2018[weird.pts,],
       pch = 1, lwd = 2,
       col = alpha(rainbow(length(weird.pts)),
                   0.75))
hist(mod.dat$LONGNUM[mod.dat$survey == surveys[1]])
hist(mod.dat$LATNUM[mod.dat$survey == surveys[1]])
mod.dat[mod.dat$survey == surveys[1] & mod.dat$LONGNUM < 5,]
table(mod.dat$cluster[mod.dat$survey == surveys[1] & mod.dat$LONGNUM < 5])
table(mod.dat$cluster[mod.dat$LONGNUM < 5])

mod.dat <- mod.dat[mod.dat$cluster != 179,]
save(mod.dat, 
