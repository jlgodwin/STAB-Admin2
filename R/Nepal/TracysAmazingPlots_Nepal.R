...
testing <- FALSE
country_vt <- c("Nepal") 
admin_vt <- c("admin1","admin2")[2] ## admin level
level_vt <- c(1,2)[2] ## polygon level
  ...
  for(admin in admin_vt){
    ...
    if(admin == "admin1"){
      map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                         layer = "npl_admbnda_adm1_nd_20180806", 
                         stringsAsFactors = F)
    }else{
      map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                         layer = "npl_admbnda_districts_nd_20180806", 
                         stringsAsFactors = F)
    }
    load(paste0(folder.name,
                shapes.sub.dir,
    ...
    for(timemod in timemod_vt){
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          if (admin == "admin1") {
            admin_name_dt$nameToPlot <- paste0("Province ",
                                               map_shp$ADM1_EN)
          } else if (admin == "admin2") {
            admin_name_dt$nameToPlot <- paste0(map_shp$DIST_EN,
                                               ",\n",
                                               "Province ",
                                               map_shp$ADM1_EN)
          }
      ...
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          for(K in K_vt){
            ...
            if (admin == "admin1") {
              namesToPlot <- paste0("Province ",
                                    map_shp$ADM1_EN)
            } else if (admin == "admin2") {
              namesToPlot <- paste0(map_shp$DIST_EN,
                                    ",\n",
                                    "Province ",
                                    map_shp$ADM1_EN)
            }
...
for (country in country_vt){
  ...
  for(admin in admin_vt){
    ...
    # set plotting names (different for admin 1 and 2)
    if (admin == "admin1") {
      admin_name_dt$toPlot <- paste0("Province ",
                                     map_shp$ADM1_EN)
    } else if (admin == "admin2") {
      admin_name_dt$toPlot <- paste0(map_shp$DIST_EN,
                                     ", Province ",
                                   map_shp$ADM1_EN)
    }
