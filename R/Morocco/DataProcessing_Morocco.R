...
message("This might be different for everyon!")
...
mod.dat<-mod.dat[which(mod.dat$years!=2004),] #Remove 2004 due to 0 deaths

#### Remove wandering clusters ####

points.path <- paste0(folder.name, "/dhsFlat/", points.files)
points <- readOGR(dsn = path.expand(points.path),
...
clus.no
mod.dat <- mod.dat[!(mod.dat$cluster %in% clus.no[3:4]),]
