...
surveys <- SurveyInfo$`Survey Year`[SurveyInfo$Country == country & SurveyInfo$`GPS?` == "Y"]
...
head(births.list[[1]])
head(births.list[[2]])
sort(unique(births.list[[2]]$admin1.name))
sort(unique(births.list[[2]]$admin2.name))

#### Check survey year is assigned to correct file name ####

sort(unique(births.list[[1]]$years))
sum(as.numeric(unique(as.character(births.list[[1]]$years))) > as.numeric(names(births.list)[1]))

sort(unique(births.list[[2]]$years))
sum(as.numeric(unique(as.character(births.list[[2]]$years))) > as.numeric(names(births.list)[2]))
