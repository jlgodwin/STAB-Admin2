...
folder.name <- CountryList$folderName[CountryList$Country == country_vt]
code_vt <- CountryList$gadmCode[CountryList$Country == country_vt]
  ...
  for(timemod in timemod_vt){
    for(admin in admin_vt){
      for(strat in strat_vt){
        ...
        # delete disputed territories
        load(paste0(folder.name, "/",
                    country, "_res_", timemod, "_", admin, "Bench.rda"))
        ...
        admin_name_dt <- admin_name_dt[!(admin_name_dt$GADM %in% disputed_regions)]
        
        # delete disputed territories
        all_draws <- res_obj$draws.est
        draws_list <- all_draws
        del_regions <- c()
        num_del <- 0
        for (i in 1:length(all_draws)) {
          if (!(all_draws[[i]]$region %in% as.character(admin_name_dt$Internal))) {
            del_regions <- c(del_regions, draws_list[[i-num_del]]$region)
            draws_list[[i-num_del]] <- NULL
            num_del <- num_del + 1
          }
        }
        ...
        save(postsamp_mt_list, 
             file = paste0(folder.name,
                           "/", country, "_",
                           timemod, "_", admin,
                           "_", strat, "_postsamp_excluding_disputed.RData"))
  ...
  for(admin in admin_vt){
    ...
    map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                       layer = paste0("gadm36_", country_code_dt[Country == country, code], "_", 
                                      admin_level_dt[Admin == admin, level],"_excluding_disputed"), 
                       stringsAsFactors = F)
    ...
    for(timemod in timemod_vt){
      for(strat in strat_vt){
        ...
        load(paste0(folder.name, "/", country, "_",
                    timemod, "_", admin, "_",
                    strat, "_postsamp_excluding_disputed.Rdata"))
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   ...
                   "_", strat, "_mean_excluding_disputed.pdf"),
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   country, "_", timemod, "_", admin,
                   "_", strat, "_median_excluding_disputed.pdf"),
               width = 7.5, height = 10)
        data_plot_dt_df <- as.data.frame(data_plot_dt)
       ...
       print(SUMMER::mapPlot(data = data_plot_dt_df, is.long = T,
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   country, "_", timemod, "_", admin,
                   "_", strat, "_wid90_excluding_disputed.pdf"),
               width = 10, height = 3.5*rowcount)
        data_plot_dt_df <- as.data.frame(data_plot_dt)
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   country, "_", timemod, "_",
                   admin, "_", strat, "_2019_median_excluding_disputed.pdf"),
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   country, "_", timemod, "_",
                   admin, "_", strat, "_2009_median_excluding_disputed.pdf"),
            width = 3.5, height = 3.5)
        {
          data_plot_dt_df <- as.data.frame(data_plot_dt)
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   country, "_", timemod, "_",
                   admin, "_", strat, "_1999_median_excluding_disputed.pdf"),
            width = 3.5, height = 3.5)
        {
          data_plot_dt_df <- as.data.frame(data_plot_dt)
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   country, "_", timemod, "_",
                   admin, "_", strat, "_1990_median_excluding_disputed.pdf"),
            width = 3.5, height = 3.5)
        {
          data_plot_dt_df <- as.data.frame(data_plot_dt)
        ...
        pdf(paste0(folder.name, "/Plots/Betabinomial/map/",
                   country, "_", timemod, "_",
                   admin, "_", strat, "_19902019_median_excluding_disputed.pdf"),
            width = 7, height = 7)
        {
          data_plot_dt_df <- as.data.frame(data_plot_dt)
          ...
          for(K in K_vt){
           ...
           #### measure map ####
            ...
            pdf(paste0(folder.name, "/Plots/Betabinomial/measure/",
                       country, "_", timemod, "_", admin,
                       "_", strat, "_Y", year,
                       "_K", K, "_measuremap_excluding_disputed.pdf"),
                   width = 4, height = 4)
            ...
            pdf(paste0(folder.name,
                          ...
                          country, "_", timemod, "_", admin,
                          "_", strat, "_Y", year, "_K",
                          K, "_measurehist_excluding_disputed.pdf"),
                   width = 4, height = 4)
            par(mar = c(4, 4, 2, 2))
        ...
        for(year in year_vt){
         ...
         #### rank postsamps ####
          ...
          pdf(paste0(folder.name, "/Plots/Betabinomial/rank/",
                     country, "_", timemod, "_",
                     admin, "_", strat,
                     "_Y", year, "_rankupp5_excluding_disputed.pdf"),
                 width = 2.5, height = 5)
          ...
          pdf(paste0(folder.name, "/Plots/Betabinomial/rank/",
                     country, "_", timemod, "_",
                     admin, "_", strat,
                     "_Y", year, "_ranklow5_excluding_disputed.pdf"),
                 width = 2.5, height = 5)
          par(mar = c(2.5, 1, 2, 1), mfrow = c(5, 1))
          ...
          pdf(paste0(folder.name, "/Plots/Betabinomial/rank/",
                     country, "_", timemod, "_",
                     admin, "_", strat,
                     "_Y", year, "_rankall_excluding_disputed.pdf"),
    ...
    for(timemod in timemod_vt){
      for(strat in strat_vt){
        ...
        pdf(paste0(country, "/Plots/Betabinomial/ridge/",
                   country, "_", timemod, "_",
                   admin, "_", strat, "_ridgeplot_excluding_disputed.pdf"),
        ...
        pdf(paste0(country, "/Plots/Betabinomial/ridge/",
                   ...
                   timemod, "_",
                   admin, "_",
                   strat, "_2019_ridgeplot_excluding_disputed.pdf"),
