...
if(doHIVAdj){
  ...
  for(survey in surveys){
    ...
    if(country == "Kenya"){
      hiv.shape <- readOGR(dsn = 'Kenya/shapeFiles_KE2014DHS/shps',
    ...
    for(area in admin1.names$GADM){
      if(natl.unaids){
        ...
        if(country == "Kenya"){
          if(area == "Elgeyo-Marakwet"){
            area.tmp <- gsub("-", " ", area)
          }else if(area == "Trans Nzoia"){
            area.tmp <- gsub(" ", "-", area)
          }else{
            area.tmp <- area
          }
          area.hiv <- hiv.shape@data$OTHREGNA[hiv.shape@data$DHSREGEN == area.tmp]
          area.hiv <- gsub(" ", "", area.hiv)
          adj.frame.tmp <- adj.frame[tolower(adj.frame$area) == area.hiv &
                                       adj.frame$years %in%
                                       seq(beg.year+2, end.year-2, 5), ]
        }else{
          adj.frame.tmp <- adj.frame[tolower(adj.frame$area) == area &
                                       adj.frame$years %in%
                                       seq(beg.year+2, end.year-2, 5), ]
        }
