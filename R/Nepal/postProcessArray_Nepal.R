...
load(paste0(folder.name, 
            shapes.sub.dir,
            '/npl/', country, '_Amat.rda'))
load(paste0(folder.name, 
            shapes.sub.dir,
            '/npl/', country, '_Amat_Names.rda'))
    ...
    height = 8, width = 8)
{
  ...
  if (nrow(tmp.area) >0 & sum(is.na(tmp.area$mean)) == nrow(tmp.area)) {
    ...
    lines(res.tmp$years[1:length(beg.year:max(mod.dat$years))], 
          res.tmp$median[1:length(beg.year:max(mod.dat$years))],
          col = 'black',
          lwd = 2, lty = 1)
           ...
           country, '_rw2_natl_temporal.pdf'), height = 5, width = 7.5)
{
  ...
  legend('bottomleft', cex = 0.65, lty = 1,
         lwd = 2,
         col = c(age.cols),
         legend = c(ages), bty = 'n')
}
    ...
    for(survey in surveys){
      ...
      if(svy.idx== 1){
        if(dim(tmp)[1] != 0){
          ...
          ihme.years <- (tmp.ihme$year)
          lines(ihme.years, tmp.ihme$mean, lwd = 2, col  = cols[9])
          #lines(ihme.years, tmp.ihme$lower, lwd = 1, lty = 2, col  = cols[length(surveys)+1])
          #lines(ihme.years, tmp.ihme$upper, lwd = 1, lty = 2, col  = cols[length(surveys)+1])
           ...
           country, '_rw2_natlBench_temporal.pdf'), height = 5, width = 7.5)
{
  ...
  for(age in ages){
    ...
    lines(beg.year:end.year, tmp$median + fixed.eff,
          col = age.cols[match(age, ages)], lty = 1, lwd = 2)
    
  }
  
  legend('topright', cex = 0.6, lty = 1, lwd = 2,
         col = c(age.cols),
         legend = c(ages), bty = 'n')
}
dev.off()
...
if(useHIVAdj){
  ...
  write.csv(direct.admin1,
            file = paste0(folder.name, '/',
                          country, '_direct_admin1.csv'),
            row.names = FALSE)
  load(paste0(folder.name, '/',
              country, '_directHIV_admin1.rda'), envir = .GlobalEnv)
  write.csv(direct.admin1,
            file = paste0(folder.name, '/',
                          country, '_directHIV_admin1.csv'),
            row.names = FALSE)
}else{
  load(paste0(folder.name, '/',
              country, '_direct_admin1.rda'), envir = .GlobalEnv)
...
load(paste0(folder.name,'/npl/', country,
            '_res_', time.mod, '_admin1.rda'))
write.csv(res.admin1$overall,
          file = paste0(folder.name, '/npl/',
                        country, '_res_', time.mod, '_admin1.csv'),
          row.names = FALSE)
load(paste0(folder.name,'/npl/', country, '_', 
            time.mod, '_admin1Benchmarks.rda'))
load(paste0(folder.name, '/npl/', country, 
            '_', time.mod,'_admin1_noStrata_temporals.rda'))
load(paste0(folder.name, '/npl/',
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
  ...
  for(area in admin1.names$Internal){
    ...
    if (nrow(tmp.area) >0 & sum(is.na(tmp.area$mean)) == nrow(tmp.area)) {
      ...
      lines(pane.years[max(est.ids):length(pane.years)], 
            ...
            col = cols[11],
            lwd = 2, lty = 2)
      legend('topright', bty = 'n', col = c(cols[1:length(surveys)],
                                            cols[9], cols[11], 'black'),
             lwd = 2, lty = c(rep(1, length(surveys)+3)),
             legend = c(survey.legends, 'IHME', 'Smoothed Direct', 'Betabinomial'),
             cex = 0.6)
      ...
      #      }
      legend('topright', bty = 'n', fill = alpha(c(cols[c(9,11)],
                                                   'black'), .25),
...
write.csv(res.admin1$overall,
          file = paste0(folder.name, '/npl/',
                        country, '_res_', time.mod, '_admin1Bench.csv'),
          row.names = FALSE)
load(paste0(folder.name,'/npl/', country, '_', 
            time.mod, '_admin1Benchmarks.rda'))
load(paste0(folder.name, '/npl/', country, 
    ...
    if (nrow(tmp.area) >0 & sum(is.na(tmp.area$mean)) == nrow(tmp.area)) {
      ...
      legend('topright', bty = 'n', col = c(cols[1:length(surveys)],
             ...
             lwd = c(rep(1, length(surveys)),
                     2, 2, 2),
             lty = c(rep(1, length(surveys)+3)),
             legend = c(survey.legends, 'IHME',
                        'Smoothed Direct', 'Betabinomial'),
             cex = 0.6)
      ...
      lines(tmp.area$years.num[(length(beg.year:max(mod.dat$years))+1):length(beg.year:end.year)], 
            ...
            col = 'black', 
            lwd = 2, lty = 2)
      legend('topright', bty = 'n', fill = alpha(c(cols[9],cols[11],
                                                   'black'), .25),
             border = c(cols[9],cols[11],
                        'black'),
             legend = c('IHME', 'Smoothed Direct', 'Betabinomial'), cex = 0.75)
    }
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_', time.mod,
           '_admin1Bench_noStrata_spaghetti_noIHME_6per.pdf'),
    height = 9, width = 6)
{
  par(mfrow = c(3,2),lend=1)
  area.idx <- 0
...
if(useHIVAdj){
  ...
  write.csv(direct.admin2,
            file = paste0(folder.name, '/',
                          country, '_direct_admin2.csv'),
            row.names = FALSE)
  load(paste0(folder.name, '/',
              country, '_directHIV_admin2.rda'), envir = .GlobalEnv)
  write.csv(direct.admin2,
            file = paste0(folder.name, '/',
                          country, '_directHIV_admin2.csv'),
            row.names = FALSE)
}else{
  load(paste0(folder.name, '/',
              country, '_direct_admin2.rda'), envir = .GlobalEnv)
...
write.csv(res.admin2$overall,
          file = paste0(folder.name, '/npl/',
                        country, '_res_', time.mod, '_admin2.csv'),
          row.names = FALSE)
load(paste0(folder.name,'/npl/', country, '_', 
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_', time.mod, '_admin2_noStrata_spaghetti_6per.pdf'),
    height = 9, width = 6)
{
  par(mfrow = c(3,2),lend=1)
  area.idx <- 0
  for(area in admin2.names$Internal){
    ...
    if (nrow(tmp.area) >0 & sum(is.na(tmp.area$mean)) == nrow(tmp.area)) {
      ...
      legend('topright', bty = 'n', col = c(cols[1:length(surveys)], 'black'),
             lwd = 2, lty = c(rep(1, length(cols)+1)),
             legend = c(survey.legends, 'IHME', 'Betabinomial'),
             cex = 0.6)
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_', time.mod,
           '_admin2_noStrata_spaghettiSingle.pdf'),
    height = 5, width = 5)
{
  area.idx <- 0
  for(area in admin2.names$Internal){
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_', time.mod,
           '_admin2_noStrata_spaghettiSingle_noIHME.pdf'),
    height = 5, width = 5)
{
  area.idx <- 0
  for(area in admin2.names$Internal){
...
order.frame <- res.admin2$overall[res.admin2$overall$years == 2019,]
areaOrder <- order.frame$region[order(order.frame$median)]
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_', time.mod,
  ...
  for(area in areaOrder){
    ...
    if (nrow(tmp.area) >0 & sum(is.na(tmp.area$mean)) == nrow(tmp.area)) {
      ...
      polygon(x = c(beg.year:end.year,
              ...
              col = alpha('black', 0.25),
              border = FALSE)
      legend('topright', bty = 'n', col = c(cols[c(1:length(surveys), 9)], 'black'),
             lwd = 2, lty = c(rep(1, length(surveys)+2)),
             legend = c(survey.legends,'IHME', "Betabinomial"),
             cex = 0.6)
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_',
           time.mod, 
           '_admin2_spaghetti_allByMedian.pdf'),
    height = 6, width = 6)
par(lend=1)
for (i in 1:numberPages) {
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_', 
           time.mod,
           '_admin2_spaghetti_all_GADMorder.pdf'))
par(lend=1)
    ...
    if (nrow(tmp.area) >0 & sum(is.na(tmp.area$mean)) == nrow(tmp.area)) {
      ...
      legend('topright', bty = 'n', fill = alpha(c(cols[c(9,11)],
             ...
             border = c(cols[c(9,11)],
                        'black'),
             legend = c('IHME',
                        'Smoothed Direct', 'Betabinomial'), cex = 0.75)
    }
...
pdf(paste0(folder.name, '/Plots/npl/Betabinomial/',
           country, '_', time.mod,
           '_admin2Bench_noStrata_spaghetti_noIHME_6per.pdf'),
    height = 9, width = 6)
{
  par(mfrow = c(3,2),lend=1)
  area.idx <- 0
    ...
    if (nrow(tmp.area) >0 & sum(is.na(tmp.area$mean)) == nrow(tmp.area)) {
      plot(NA,
           ...
           main = paste(admin2.names$GADM[area.idx],
                        poly.adm2$NAME_1[area.idx], 
                        sep = ", "))
      legend('topright', bty = 'n', col = c(cols[1:length(legends)], 'black'),
             lwd = 2, lty = 1, legend = c(survey.legends, "Betabinomial"))
