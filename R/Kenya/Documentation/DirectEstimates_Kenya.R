  ...
  if(subnatl.unaids){
    ...
    for(survey in surveys){
      for(i in 1:dim(file.name.key)[1]){
        ...
        file.in <- file.list.per[grepl(paste0(file.names$File, '_'), file.list.per)]
    ...
    for(survey in surveys){
      ...
      if(is.factor(hiv.adj.yearly[,1])){
        hiv.adj.yearly[,1] <- as.numeric(as.character(hiv.adj.yearly[,1]))
      }
      if(is.factor(hiv.adj.yearly[,2])){
        hiv.adj.yearly[,2] <- as.numeric(as.character(hiv.adj.yearly[,2]))
      }
      years.before.mm <- as.numeric(hiv.adj.yearly[-1 ,1])
