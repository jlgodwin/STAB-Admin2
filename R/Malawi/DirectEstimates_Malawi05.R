...
## Admin1 Direct ####
if(length(births.list) != 1){
  direct.admin1 <-  getDirectList(births.list, beg.year:end.year,
                                  regionVar = "admin0.5.char",
                                  timeVar = "years", 
                                  clusterVar =  "~cluster",
                                  ageVar = "age", Ntrials = "total",
...
if(doHIVAdj){
  ...
  for(survey in surveys){
    ...
    for(area in admin0.5.names$GADM){
      if(natl.unaids){
        adj.frame.tmp <- adj.frame
        adj.frame.tmp <- adj.frame[adj.frame$years %in% beg.year:end.year, ]
       # adj.frame.tmp$years <- periods
        ...
        adj.frame.tmp <- adj.frame[adj.frame$area == area &
                                     adj.frame$years %in% seq(beg.year+2,end.year-2,5), ]
        adj.frame.tmp$years <- periods
      }
      
      area.int <- admin0.5.names$Internal[match(area, admin0.5.names$GADM)]
      ...
      if(exists('direct.admin2')){
        admin2.to.admin1 <- data.frame(GADM.adm2 = poly.adm2@data$NAME_2,
                                       ...
                                       Internal.adm1 = NA)
        admin2.to.admin1$Internal.adm1 <- admin1.names$Internal[match(admin2.to.admin1$GADM.adm1, admin1.names$GADM)]
        ...
        direct.admin2[direct.admin2$region %in% admin2s &
                        direct.admin2$surveyYears == survey,] <- tmp.adj[ , match(colnames(direct.admin2), 
                                                                                  colnames(tmp.adj))]
      }
    }
  }
  ...
  save(direct.admin1, file = paste0(folder.name, '/',
                                    country, '_directHIV_admin0.5.rda'))
  if(exists('direct.admin2')){
...
for(survey in surveys){
  png(paste0(folder.name,"/Plots/Direct/", country, '_admin0.5_', 
             survey,'_direct_poly.png'))
...
par(mfrow = c(1,1))
pdf(paste0(folder.name,"/Plots/Direct/", country, '_admin0.5_direct_spaghetti.pdf'))
...
for(area in 1:dim(poly.adm1)[1]){
  ...
  tmp.ihme <- ihme.ests[[2]][ihme.ests[[2]]$ADM1_NAME == as.character(admin0.5.names$GADM[area]),]
...
if(doAdmin2){
  ...
  for(area in 1:dim(poly.adm2)[1]){
    tmp.area <- direct.admin2[direct.admin2$region == as.character(admin2.names$Internal[area]),]
    tmp.area$width <- tmp.area$upper - tmp.area$lower
    tmp.area$cex2 <- median(tmp.area$width, na.rm = T)/tmp.area$width
    tmp.area$cex2[tmp.area$cex2 > 6] <- 6
