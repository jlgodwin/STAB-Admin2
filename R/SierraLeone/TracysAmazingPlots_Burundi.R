...
testing <- FALSE
country_vt <- c("Burundi") 
admin_vt <- c("admin1","admin2")[1:2] ## admin level
level_vt <- c(1,2)[1:2] ## polygon level
...
folder.names <- CountryList$folderName[CountryList$Country == country_vt]
code_vt <- CountryList$gadmCode[CountryList$Country == country_vt]

country_code_dt <- data.table::data.table(Country = country_vt,
                              code = code_vt)

admin_level_dt <- data.table::data.table(Admin = admin_vt,
                             level = level_vt)
...
for (country in country_vt){
  ...
  for(admin in admin_vt){
    ...
    if(admin == "admin2"){
      map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                         layer = 'bdi_admbnda_adm2_igebu_ocha_20171103',
                         stringsAsFactors = F)
    ...
    admin_name_dt <- data.table::as.data.table(get(paste0(admin, ".names")))
    ...
    for(timemod in timemod_vt){
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          if (admin == "admin1") {
            admin_name_dt$nameToPlot <- map_shp$admin1Name
            name.string <- "admin1Name"
          } else if (admin == "admin2") {
            admin_name_dt$nameToPlot <- paste0(map_shp$admin2Name, ",\n", map_shp$admin1Name)
            name.string <- "admin2Name"
          }
          
          # create data to plot
          data_plot_dt_year <- data.table::data.table(Year = year, 
          ...
          data_plot_dt_year[, 
                            paste0("NAME_", admin_level_dt[Admin == admin, level]) := GADM]
      ...
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          for(K in K_vt){
            ...
            if (admin == "admin1") {
              namesToPlot <- map_shp$admin1Name
            } else if (admin == "admin2") {
              namesToPlot <- paste0(map_shp$admin2Name,
                                    ",\n", map_shp$admin1Name)
            }
            
            measure_dt <- get_measure_dt(postsamp_mt = postsamp_mt,
                                         pred_dt = data.table::data.table(adm_name = paste0(admin, "_", 1:nrow(postsamp_mt)),
      ...
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          for (i in 1:5){
            ...
            ranktable <- data.table::as.data.table(table(rank_vt))
            ranktable <- merge(data.table::data.table(rank = as.character(1:10)), ranktable, 
          ...
          for (i in 1:5){
            ...
            ranktable <- data.table::as.data.table(table(rank_vt))
            ranktable <- merge(data.table::data.table(rank = as.character(nrow(pred_dt_order):(nrow(pred_dt_order)-9))), ranktable, 
                               by.x = "rank", by.y = "rank_vt", all.x = T)
            ranktable[, "rank" := as.integer(rank)]
            ranktable <- ranktable[order(rank)]
            ranktable[is.na(N), "N"] <- 0
          ...
          for (i in 1:nrow(pred_dt_order)){
            ...
            ranktable <- data.table::as.data.table(table(rank_vt))
            ranktable <- merge(data.table::data.table(rank = as.character(1:nrow(pred_dt_order))), ranktable, 
                               by.x = "rank", by.y = "rank_vt", all.x = T)
            ranktable[, "rank" := as.integer(rank)]
            ranktable <- ranktable[order(rank)]
            ranktable[is.na(N), "N"] <- 0
