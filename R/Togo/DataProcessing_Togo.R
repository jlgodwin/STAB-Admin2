...
#  
#  sources: code.dir.rel/LoadCommandCenter.R
#  reads:   code.dir.rel/SurveyNum.csv
#           code.dir.rel/SurveyList_BR_GE.csv
# 
...
for(i in 1:n.survey){
  ...
  br.name.root <- gsub("DT.ZIP", "",
                       ignore.case = T, br.name.zip)
  br.name.root <- toupper(br.name.root)
  births.files[i] <- paste0(br.name.root, "DT/",
                            br.name.root, "FL.dta")
} 
...
### Check if GADM Polygon files contain water ####
...
if(adm2.ind){
  ...
  colnames(admin2.mat) <- 
    rownames(admin2.mat) <- paste0("admin2_",
                                   1:dim(admin2.mat)[1])
  admin2.names <- data.frame(GADM = poly.adm2@data$PREF,
...
for(survey in surveys){
  svy.idx <- svy.idx + 1
  dat.tmp <- SUMMER::getBirths(filepath = paste0(folder.name,
                                         '/dhsStata/',
                                         filenames[svy.idx]),
                       surveyyear = survey,
  ...
  for(i in 1:dim(points)[1]){
    dat.tmp$LATNUM[dat.tmp$v001 == 
                     points@data$DHSCLUST[i]] <- points@data$LATNUM[i]
    dat.tmp$LONGNUM[dat.tmp$v001 == 
                      points@data$DHSCLUST[i]] <- points@data$LONGNUM[i]
  }
  ...
  if(adm2.ind){
    ...
    dat.tmp$admin2.name <- as.character(poly.adm2@data$PREF)[admin2.key]
  }else{
