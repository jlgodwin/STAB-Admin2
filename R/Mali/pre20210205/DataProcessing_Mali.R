...
births.files <- gps.files <- SvyIds.idx <- rep(NA, n.survey)
...
if(!file.exists(paste0(poly.path,'/', country, '_Amat.rda'))){
    ...
    if(adm1.ind){
        ...
        rownames(admin1.mat) <- poly.adm1@data$NAME_1
    }else{
    ...
    if(adm2.ind){
        ...
        rownames(admin2.mat) <- poly.adm2@data$NAME_2
    }else{
    ...
    if(adm2.ind){
        ...
        if(dim(miss.frame.adm2)[1] != 0){
            ...
            for(i in 1:dim(miss.poly.adm2)[1]){
                ids <- which(points.frame@coords[,c("LONGNUM", "LATNUM")] %in% miss.frame.adm2[i,])
    ...
    if(adm1.ind){
        ...
        if(dim(miss.frame.adm1)[1] != 0){
            ...
            for(i in 1:dim(miss.poly.adm1)[1]){
                ids <- which(points.frame@coords[,c("LONGNUM", "LATNUM")] %in% miss.frame.adm1[i,])
                ids[(length(ids)/2 + 1):length(ids)] <- ids[(length(ids)/2 + 1):length(ids)] - dim(points.frame@coords)[1]
                ids <- unique(ids)
...
births.list <- lapply(births.list, function(x){
    tmp <- x[,c("v001", "age", "time", "total", "died", "v005", 
                ...
                "admin1", "admin2", "admin1.name", "admin2.name")]
...
for (i in 1:length(births.list)) {
    cat(paste("\n",surveys[i],"\n\n"))
    # print(head(births.list[[i]]))
    # print(tail(births.list[[i]]))
    # 
    # sort(unique(births.list[[i]]$years))
    
    #### Check survey year is assigned to correct file name ####
    print(sum(as.numeric(unique(as.character(births.list[[i]]$years))) > as.numeric(names(births.list)[i])))
}

head(births.list[[4]][births.list[[4]]$years==2001,])
