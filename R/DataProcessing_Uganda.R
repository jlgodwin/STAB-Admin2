...
merge.dat <- poly.adm2@data %>% 
  group_by(ADM1_EN) %>%
  summarise(n = n(), 
            ADM1_PCODE = unique(ADM1_PCODE))
poly.adm1 <- SpatialPolygonsDataFrame(poly.adm1,
                                      merge.dat)
...
if(adm2.ind){
  ...
  colnames(admin2.mat) <- 
    rownames(admin2.mat) <- paste0("admin2_",
                                   1:dim(admin2.mat)[1])
  admin2.names <- data.frame(GADM = poly.adm2@data$ADM2_EN,
...
for(survey in surveys){
  ...
  if(adm2.ind){
    ...
    dat.tmp$admin2.name <- as.character(poly.adm2@data$ADM2_EN)[admin2.key]
  }else{
