...
nat_1year <- function(svy_data,year,
  ...
  if(mean(as.numeric(svy_data$v025))==2){
    svy_data<-add_ghost(svy_data,year = year,1)
  }
  if(mean(as.numeric(svy_data$v025))==1){
  ...
  for (i in 1:length(ageseg)){
    ...
    age_data$urban_indicator<- -(as.numeric(age_data$v025)-2)
...
svy_process<-function(c_svy){
  ...
  if(max(c_svy$urban_prop,na.rm=TRUE)==1){
  ...
  if(min(c_svy$urban_prop,na.rm=TRUE)==0){
...
comb_1year_1age_band<-function(c_svy,year,age_band=NULL){
  ...
  if (min(svy_year$urban_prop,na.rm=TRUE)==0){
    logit_prop<-NA
    urban_prop<-0
    logit_se<-NA
    SE<-0
...
## edit weights variable for Pakistan (sv005 has the weights for Azad Kashmir and Northern Areas)
svy_list[[2]]$v005 <- svy_list[[2]]$v005 + svy_list[[2]]$sv005
