  ...
  for(admin in admin_vt){
    ...
    if(admin == "admin1"){
      map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                         layer = paste0("sle_adm1_province_mic"), 
                       stringsAsFactors = F)
    }
    
    if(admin == "admin2"){
      map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                         layer = paste0("sle_adm2_districts_mic_cleaned"), 
                         stringsAsFactors = F)
    }
    load(paste0(folder.name,
                shapes.sub.dir,
    ...
    for(timemod in timemod_vt){
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          if (admin == "admin1") {
            admin_name_dt$nameToPlot <- map_shp$province
            geo.name <- "province"
          } else if (admin == "admin2") {
            admin_name_dt$nameToPlot <- paste0(map_shp$new_distri,
                                               ",\n", map_shp$district_2)
            geo.name <- "new_distric"
          }
