...
for (country in country_vt){
  ...
  for(admin in admin_vt){
    ...
    admin_name_dt <- as.data.table::data.table(get(paste0(admin, ".names")))
...
get_measure_dt <- function(postsamp_mt, pred_dt, grp_thresh){
  ...
  grp_cnt_dt_max <- as.data.table::data.table(DF)
      ...
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          for (i in 1:5){
            ...
            ranktable <- as.data.table::data.table(table(rank_vt))
