...
rm(list = ls())
setwd('/Users/taylorokonek/Desktop/Africa/')
...
{
## Load IGME ####
file.list <- list.files('R')
igme.file <- file.list[grepl("IGME", file.list)]
igme.ests <- read.csv(paste0('R/',igme.file),
                      header = T)
...
ihme.ests <- list()
ihme.ests[['adm0']] <- read.csv( paste0('R/',
                                        ihme.files[grepl("ADM0", ihme.files)]),
                                 header = T)
ihme.ests[['adm1']] <- read.csv( paste0('R/',
                                        ihme.files[grepl("ADMIN1", ihme.files)]),
                                 header = T)
ihme.ests[['adm2']] <- read.csv( paste0('R/',
                                        ihme.files[grepl("ADMIN2", ihme.files)]),
                                 header = T)

# country needs to be "Swaziland" at this point
country <- "Swaziland"
...
country <- "Eswatini"
source(paste0(folder.name,
              '/IHMEHand_', country, '.R'))

#changed country back to eswatini
  ...
  if(subnatl.unaids){
    ...
    for(survey in surveys){
      ...
      hiv.adj.yearly <- read.csv(paste0(folder.name,
                                        '/HIV/', country, '_National_', survey, '_Yearly.csv'),
                                 header = T)
...
for(area in 1:dim(poly.adm1)[1]){
  ...
  tmp.ihme <- ihme.ests[[2]][ihme.ests[[2]]$ADM1_NAME == as.character(admin1.names$GADM[area]),]
