...
if(doHIVAdj){
  ...
  if(subnatl.unaids){
    ...
    file.name.key <- data.frame(File = NA,
                                GADM = sort(admin1.names$GADM),
                                Internal = admin1.names$Internal)
    
    i <- NULL
    for(i in 1:dim(file.name.key)[1]){
      sum <- summary(HIVnames.key$dhs.name[HIVnames.key$admin1.Internal == 
                                             as.character(file.name.key$Internal[i])])
      print(sum[sum != 0])
      file.name.key$File[i] <- names(sum)[which.max(sum)]
    }
    ...
    for(survey in surveys){
      for(i in 1:dim(file.name.key)[1]){
        ...
        file.in <- file.list.per[grepl(paste0("_",file.names$File,"_"), 
                                       file.list.per)]
