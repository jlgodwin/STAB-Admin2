...
#### Admin Half  benchmark ####
 
 load(paste0(folder.name, '/',
             country, '_res_admin0.5Bench_SmoothedDirect.rda'))
 res.smoothdir <- res.admin1
 
 # reloading these in case you're just running the benchmark section of the code
 if(useHIVAdj){
   load(paste0(folder.name, '/',
               country, '_directHIV_admin0.5.rda'), envir = .GlobalEnv)
 }else{
   load(paste0(folder.name, '/',
               country, '_direct_admin0.5.rda'), envir = .GlobalEnv)
 }
# 
