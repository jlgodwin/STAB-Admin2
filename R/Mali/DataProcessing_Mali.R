...
weird.pts <- which(mod.dat$LATNUM < 5)
unique(mod.dat$cluster[weird.pts])

plot(unique(mod.dat[mod.dat$survey == 2018,c("LONGNUM", "LATNUM")]),
    axes = FALSE,
    pch = 16,
    col = 'grey88',
    main = '')
plot(poly.adm0, add = TRUE)
points(unique(mod.dat[weird.pts, c("LONGNUM", "LATNUM")]),
       lwd = 2, col = 'red')


hist(mod.dat$LONGNUM[weird.pts],
     xlab = "LONGNUM",
     main = 'Points outside country')
hist(mod.dat$LATNUM[weird.pts],
     xlab = "LATNUM",
     main = 'Points outside country')


mod.dat <- mod.dat[-weird.pts,]
