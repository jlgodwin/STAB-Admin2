...
if(useHIVAdj){
  if(sum(grep("HIV_", files.list)) == 0){
    ...
    load(paste0(folder.name, '/', country, '_direct_admin1_yearly.rda'))
  }else{
    ...
    load(paste0(folder.name, '/', country, '_directHIV_admin1_yearly.rda'))
  }
}else{
  load(paste0(folder.name, '/', country, '_direct_natl_yearly.rda'))
  load(paste0(folder.name, '/', country, '_direct_natl.rda'))
...
data.admin1.yearly <- aggregateSurvey(direct.admin1.yearly)
...
fit.admin1.yearly <- smoothDirect(data.admin1.yearly,
                                  Amat = admin1.mat,
                                  type.st = 4,
                                  ...
                                  is.yearly = F)
res.admin1.yearly <- getSmoothed(fit.admin1.yearly,
                                 Amat = admin1.mat,
                                 year_label = beg.year:end.year,
                                 year_range = c(beg.year, end.year),
                                 include_time_unstruct = FALSE,
                                 save.draws = TRUE)
res.admin1.yearly$results$years.num <- res.admin1.yearly$results$years
res.admin1.yearly$results$region.gadm <- admin1.names$GADM[match(res.admin1.yearly$results$region,
                                                                 admin1.names$Internal)]
head(res.admin1.yearly$results)
tail(res.admin1.yearly$results)

file.out <- paste0(country,
                   "_res_admin1_yearly_SmoothedDirect.rda")
save(res.admin1.yearly,
     file = paste0(folder.name, 
                   '/', file.out))
...
fit.natl.yearly.noiid <- smoothDirect(data.natl.yearly,
                                      Amat = NULL,
                                      year_label = as.character(beg.year:(end.year + 5)),
                                      year_range = c(beg.year, end.year + 5),
                                      include_time_unstruct = FALSE,
                                      is.yearly = F) 
res.natl.yearly.noiid <- getSmoothed(fit.natl.yearly.noiid,
                                     year_range = c(beg.year, end.year + 5),
                                     year_label = as.character(beg.year:(end.year + 5)))

benchmark <- data.frame(years = beg.year:end.year,
                        ratio = NA)
benchmark$ratio<- (res.natl.yearly.noiid$median[match(beg.year:end.year, res.natl.yearly.noiid$years.num)])/
...
lincombs.info <- fit.admin1.yearly$lincombs.info
for(yr in beg.year:end.year){
  
  bench <- benchmark$ratio[benchmark$years == yr]
  
  for(area in rownames(admin1.mat)){
    index <- lincombs.info$Index[lincombs.info$District == match(area, 
                                                                 rownames(admin1.mat)) &
                                   lincombs.info$Year == match(yr, beg.year:end.year)]
    res.index <- which(res.admin1.yearly$results$region == area &
                         res.admin1.yearly$result$years == yr)
    
    tmp<- res.admin1.yearly$draws.est[index,]/bench
    
    
    res.admin1.yearly$results[res.index,
...
file.out <- paste0(country,
                   "_res_admin1Bench_yearly_SmoothedDirectCI90.rda")
save(res.admin1.yearly,
     file = paste0(folder.name, 
                   '/', file.out))

write.csv(res.admin1.yearly$results, row.names = FALSE,
          file = paste0(folder.name, '/',
                      country, 
                      '_res_admin1Bench_yearly_SmoothedDirectCI90.csv'))
...
pdf(paste0(folder.name,
           "/Plots/SmoothedDirect/",
           country,
           '_admin1Bench_yearly_SmoothedDirect.pdf'),
    height = 6, width = 6)
tmp.res <- res.admin1.yearly$results
...
pdf(paste0(folder.name,
           "/Plots/SmoothedDirect/",
           country,
           '_admin1Bench_yearly_SmoothedDirect_spaghetti.pdf'),
    height = 6, width = 6)
{
  par(mfrow = c(1,1),
      lend = 1)
  for(area in 1:dim(poly.adm1)[1]){
    tmp.area <- direct.admin1.yearly[direct.admin1.yearly$region == 
    ...
    res.area <- res.admin1.yearly$results[res.admin1.yearly$results$region == 
                             as.character(admin1.names$Internal[area]),]
...
pdf(paste0(folder.name,
           '/Plots/SmoothedDirect/',
           country,
           '_admin1Bench_compareIGME.pdf'),
    height = 6, width = 6)

plot.lims <- range(res.admin1.yearly$results$median*1000) + c(-5,5)
plot(beg.year:end.year,
     1000*res.admin1.yearly$results$median[res.admin1.yearly$results$region == "admin1_1"],
     ylim = plot.lims,
     xlab = "Year",
     ylab = "U5MR",
     pch = 19,
...
points(beg.year:end.year,
       1000*res.admin1.yearly$results$median[res.admin1.yearly$results$region == "admin1_2"],
       pch = 19, col = alpha(cols[2], 0.5))
points(beg.year:end.year,
       1000*res.admin1.yearly$results$median[res.admin1.yearly$results$region == "admin1_3"],
       pch = 19, col = alpha(cols[3], 0.5))
points(beg.year:end.year,
       igme.ests$OBS_VALUE,
       pch = 4, lwd = 2)
legend('bottomleft',
