...
for (country in country_vt){
  ...
  for(admin in admin_vt){
    ...
    map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                       layer = paste0('bdi_admbnda_adm',
                                      admin_level_dt[Admin == admin, level],
                                      '_igebu_ocha_20171103'), 
                       stringsAsFactors = F)
    load(paste0(folder.name,
                shapes.sub.dir,
                '/', country,
...
for (country in country_vt){
  ...
  for(admin in admin_vt){
    ...
    # set plotting names (different for admin 1 and 2)
    if (admin == "admin1") {
      admin_name_dt$toPlot <- map_shp$admin1Name
    } else if (admin == "admin2") {
      admin_name_dt$toPlot <- paste0(map_shp$admin2Name, ", ", map_shp$admin1Name)
    }
...
for (country in country_vt){
  ...
  for(admin in admin_vt){
    ...
    if (admin == "admin1") {
      ...
      admin_name_dt$toPlot <- paste0(map_shp$admin2Name, ",\n", map_shp$admin1Name)
    }
