...
for (i in 1:numberPages) {
  ...
  if (i %in%  pagesWithLessAreas) {
    nPerPage <- round((numberAreasPerPage +
                         numberAreasTotal%%numberAreasPerPage)/2, digits = 0)
    
    if(i != numberPages){
      areas <- areaOrder[(((i-1)*numberAreasPerPage)+1):(((i-1)*numberAreasPerPage)+nPerPage)]
    }else if(i == numberPages){
      areas <- areaOrder[(((i-2)*numberAreasPerPage)+nPerPage+1):numberAreasTotal]
    }
