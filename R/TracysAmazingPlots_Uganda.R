...
for (country in country_vt){
  ...
  for(admin in admin_vt[2]){
    ...
    map_shp <- readOGR(dsn = paste0(folder.name, shapes.sub.dir), 
                       layer = "uga_admbnda_adm2_2020", 
                       stringsAsFactors = F)
    
    if(admin == "admin1"){
      
      poly.adm1.poly <- SpatialPolygons(map_shp@polygons)
      map_shp_small <- unionSpatialPolygons(poly.adm1.poly,
                                        IDs = match(map_shp@data$ADM1_EN,
                                                    unique(map_shp@data$ADM1_EN)))
      
      merge.dat <- map_shp@data %>% 
        group_by(ADM1_EN) %>%
        summarise(n = n(), 
                  ADM1_PCODE = unique(ADM1_PCODE))
      map_shp <- SpatialPolygonsDataFrame(map_shp_small,
                                            merge.dat)
    }else{
      a <- tolower(map_shp@data$ADM2_EN)
      substr(a, 1,1) <- toupper(substr(a,1,1))
      map_shp@data$ADM2_EN <- a
    }
    ...
    for(timemod in timemod_vt){
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          if (admin == "admin1") {
            admin_name_dt$nameToPlot <- map_shp$ADM1_EN
          } else if (admin == "admin2") {
            admin_name_dt$nameToPlot <- paste0(map_shp$ADM2_EN,
                                               ",\n", map_shp$ADM1_EN)
          }
      ...
      for(strat in strat_vt){
        ...
        for(year in year_vt){
          ...
          for(K in K_vt){
            ...
            if (admin == "admin1") {
              namesToPlot <- map_shp$ADM1_EN
            } else if (admin == "admin2") {
              namesToPlot <- paste0(map_shp$ADM1_EN,
                                    ",\n", map_shp$ADM2_EN)
            }
...
for (country in country_vt){
  ...
  for(admin in admin_vt[2]){
    ...
    # set plotting names (different for admin 1 and 2)
    if (admin == "admin1") {
      admin_name_dt$toPlot <- map_shp$ADM1_EN
    } else if (admin == "admin2") {
      admin_name_dt$toPlot <- paste0(map_shp$ADM2_EN, ", ",
                                     map_shp$ADM1_EN)
    }
...
for (country in country_vt){
  ...
  for(admin in admin_vt){
    ...
    if (admin == "admin1") {
      ...
      admin_name_dt$toPlot <- paste0(map_shp$ADM1_EN, ",\n",
                                     map_shp$ADM2_EN)
    }
