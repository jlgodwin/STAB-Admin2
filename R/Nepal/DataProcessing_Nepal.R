...
SurveyMeta <- read.csv(paste0(code.dir.rel,
                              '/SurveyList_BR_GE.csv'))
SurveyIds <- unique(SurveyMeta$SurveyId[SurveyMeta$CountryName == country])
...
gps.files <- SurveyInfo$GPSFile[SurveyInfo$Country == country &
                                  SurveyInfo$`GPS?` == "Y"]
...
if(adm1.ind){
  ...
  colnames(admin1.mat) <- 
    rownames(admin1.mat) <- paste0("admin1_",
                                   1:dim(admin1.mat)[1])
  admin1.names <- data.frame(GADM = poly.adm1@data$ADM1_EN,
...
if(adm2.ind){
  ...
  colnames(admin2.mat) <- 
    rownames(admin2.mat) <- paste0("admin2_",
                                   1:dim(admin2.mat)[1])
  admin2.names <- data.frame(GADM = poly.adm2@data$DIST_EN,
...
for(survey in surveys){
  ...
  if(adm2.ind){
    ...
    dat.tmp$admin2.name <- as.character(poly.adm2@data$DIST_EN)[admin2.key]
  }else{
  ...
  if(adm1.ind){
    ...
    dat.tmp$admin1.name <- as.character(poly.adm1@data$ADM1_EN)[admin1.key]
  }else{
    dat.tmp$admin2 <- dat.tmp$admin2.name <- NA
