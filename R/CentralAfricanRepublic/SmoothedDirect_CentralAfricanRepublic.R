...
#### Admin1 Model ####
#Problems: 1990-1994, admin1_16
#2020-2024, all except admin1_3
...
if(doBenchmark){
  ...
  res.natl$years.num <- seq(beg.year+2, end.year+1, 5)
  #res.natl$years.num <- seq(beg.year+2, end.year+5, 5)
  res.natl$region.gadm <- country
  head(res.natl)
  tail(res.natl)
  ...
  #year_label = c(periods, proj.per),
  #year_range = c(beg.year, end.year + 5))
  res.admin1$years.num <- seq(beg.year+2, end.year+1, 5)[match(res.admin1$years, c(periods, proj.per))]
  ...
  res.natl.yearly<- getSmoothed(fit.natl.yearly, 
                                year_label = as.character(beg.year:(end.year-8)),
                                year_range = c(beg.year,(end.year-8)))
  res.natl.yearly$years.num <- beg.year:(end.year-8)
  #res.natl.yearly<- getSmoothed(fit.natl.yearly, 
  #                        year_label = as.character(beg.year:(end.year + 5)),
  #                        year_range = c(beg.year, (end.year + 5)))
  #res.natl.yearly$years.num <- beg.year:(end.year+5)
