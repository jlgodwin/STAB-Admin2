...
if(length(births.list) != 1){
  ...
  direct.admin1.yearly <-  getDirectList(births.list, beg.year:end.year,
                                         regionVar = "admin1.char",
                                         timeVar = "years", 
                                         clusterVar =  "~cluster",
                                         ageVar = "age", Ntrials = "total",
...
save(direct.admin1.yearly,
     file = paste0(folder.name, '/',
                   country, '_direct_admin1_yearly.rda'))
...
if(doHIVAdj){
  ...
  for(survey in surveys){
    ...
    tmp.adj <- getAdjusted(direct.admin1.yearly[direct.admin1.yearly$surveyYears == survey,],
                           ratio = adj.frame, 
                           logit.lower = NULL,
                           logit.upper = NULL,
                           prob.upper = "upper",
                           prob.lower = "lower")
    direct.admin1.yearly[direct.admin1.yearly$surveyYears == survey,] <- 
    ...
    if(natl.unaids){
      ...
      if(natl.unaids){
        ...
        adj.frame.tmp$years <- periods
          ...
          tmp.adj <- getAdjusted(direct.admin1[direct.admin1$surveyYears == survey,],
                             ...
                             prob.upper = "upper",
                             prob.lower = "lower")
      direct.admin1[direct.admin1$surveyYears == survey,] <- 
      ...
      if(exists('direct.admin2')){
        tmp.adj <- getAdjusted(direct.admin2[direct.admin2$surveyYears == survey,],
                               ...
                               prob.upper = "upper",
                               prob.lower = "lower")
        direct.admin2[direct.admin2$surveyYears == survey,] <- 
  ...
  save(direct.admin1.yearly, file = paste0(folder.name, '/',
                                    country, '_directHIV_admin1_yearly.rda'))
  if(exists('direct.admin2')){
...
med.palette <- brewer.pal(n = 3, name = "Purples")
...
pdf(paste0(folder.name,
           "/Plots/Direct/",
           country,
           '_admin1_yearly_direct_spaghetti.pdf'),
    height = 6, width = 6)
